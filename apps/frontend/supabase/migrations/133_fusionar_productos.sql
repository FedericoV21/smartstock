-- Fusión de un producto duplicado en otro (mismo tenant, proveedor y sucursal).
-- Repunta FKs, suma stock_sucursal, aplica p_campos (survivor|loser) y elimina el perdedor.
-- v1: exactamente un perdedor en p_loser_ids (semántica de preferencias campo a campo).
...
BEGIN
  ...
  IF v_plu IS NOT NULL AND v_es_pesable IS NOT TRUE THEN
    RAISE EXCEPTION 'Si hay PLU el producto debe ser pesable (es_pesable)';
  END IF;
