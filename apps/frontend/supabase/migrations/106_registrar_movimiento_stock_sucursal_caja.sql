-- V110 / fase 5 — El stock de venta/compra se mueve en `stock_sucursal` del depósito del movimiento (`p_sucursal_id`),
-- que debe coincidir con la sucursal del comprobante o la caja, no forzosamente con `producto.sucursal_id` (hogar del artículo).
-- Alineado con POS `buscar-por-barcode` tenant-wide y filas `stock_sucursal` por depósito.
