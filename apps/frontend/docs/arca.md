...
## Cliente HTTP y TLS hacia AFIP (Node.js, OpenSSL 3, Vercel)

Los endpoints de producción de AFIP/ARCA pueden negociar TLS con **grupos Diffie-Hellman efímeros (DHE) de 1024 bits** u otros parámetros considerados *legacy*. En el servidor de la app (**Node 18+** con **OpenSSL 3**, p. ej. en **Vercel**), el handshake puede fallar con:

- `SSL routines:tls_process_ske_dhe:dh key too small`
- `code=EPROTO` / `write EPROTO`

Aunque exista un umbral `minDHSize` a nivel de Node, **OpenSSL 3 aplica además un SECLEVEL estricto** (por defecto o según la versión de Node, p. ej. a partir de **Node 24+**) que **rechaza el DH del servidor antes** de que la lógica de `minDHSize` baste sola. Por eso `fetch` global u opciones mínimas no son fiables para estos hosts.

**Implementación en Nexus** (solo salidas a WSAA/WSFE):

| Pieza | Archivo | Rol |
|---|---|---|
| Cliente dedicado | `src/lib/facturacion/arca/afip-https-client.ts` | `afipHttpsPost()` con `import https from 'https'` y **`https.request()`** (no `fetch` de undici) |
| Opciones TLS | Mismo módulo | `ciphers: 'DEFAULT@SECLEVEL=1'` (baja el SECLEVEL de OpenSSL **solo en esa conexión**) y `minDHSize: 512` (capa Node) |
| WSAA | `src/lib/facturacion/arca/wsaa.ts` | POST SOAP vía `afipHttpsPost` |
| WSFE | `src/lib/facturacion/arca/fetch-wsfe-retry.ts` | Reintentos + POST vía `afipHttpsPost` |

- **Alcance:** solo estas peticiones a `*.afip.gov.ar` / `*.afip.gob.ar` definidas en `endpoints.ts`. El resto del tráfico HTTPS de la app no se modifica; **no** se deshabilita la verificación del certificado del servidor (`rejectUnauthorized` sigue en el comportamiento por defecto, seguro).
- **Supabase Edge Function** `reintentar-arca` usa `fetch` en **Deno**; el stack TLS es distinto. Si en el futuro apareciera un error análogo ahí, habría que tratarlo en ese runtime, no aplica el mismo código.

Los fragmentos de código más abajo en este documento que muestran `fetch(endpoints.wsaa)` / `fetch(endpoints.wsfe)` son **ilustrativos** del flujo SOAP; en el repositorio real las llamadas salientes usan el cliente anterior.
...
## Error WSFE 10016 — «El numero o fecha del comprobante no se corresponde con el proximo a autorizar»

Código frecuente en `FECAESolicitar` cuando número o fecha no encajan con lo que AFIP/ARCA espera para ese **punto de venta** y **tipo de comprobante** (ver [errores comunes en documentación tipo Afip SDK](https://docs.afipsdk.com/recursos/errores-frecuentes)).

| Causa | Qué revisar / mitigación en Nexus |
|-------|-----------------------------------|
| **Numeración** | El siguiente número debe ser **`FECompUltimoAutorizado + 1`** (mismo `PtoVta` y `CbteTipo`). No alcanza con el máximo local si hubo emisiones fuera de Nexus o por otro ambiente. `resolverNumeroParaSolicitudCae` usa ARCA como fuente de verdad. |
| **Fecha (`CbteFch`)** | Si la fecha enviada es **anterior** a la del último comprobante ya registrado para esa secuencia, se rechaza. Borradores POS/MP QR que quedan varios días antes de emitir pueden traer fecha vieja; al solicitar CAE se normaliza a **hoy (Argentina)** cuando la fecha del comprobante está en el pasado. |
| Homologación vs producción | Punto de venta y ambiente deben coincidir con donde se emitió el último comprobante autorizado. |

### Ticket POS vs factura con CAE

En el modelo v9, **solo** `factura_a`, `factura_b`, `factura_c` y notas de crédito fiscales pasan por `FECAESolicitar` y reciben CAE. Un comprobante **`ticket`** en el POS queda **emitido** a nivel comercial y de caja, pero **no** es una factura electrónica AFIP. Para comprobante fiscal con CAE en ARCA, hay que elegir **Factura** (no solo ticket) en el POS.
...
## Errores de red hacia WSFE (`ECONNRESET`, `fetch failed`, `EPROTO` / TLS)

Mensajes típicos: `TypeError: fetch failed → … code=ECONNRESET`, o `read ECONNRESET` al llamar a `https://servicios1.afip.gov.ar/wsfev1/service.asmx` (producción) u homologación. En Node/Vercel también puede aparecer **`dh key too small`**, `tls_process_ske_dhe` o **`write EPROTO`** (TLS) al negociar con servidores AFIP legacy; ver sección [Cliente HTTP y TLS hacia AFIP](#cliente-http-y-tls-hacia-afip-nodejs-openssl-3-vercel).
...
| Rechazo TLS (DH/SECLEVEL) en producción o tras actualizar Node en Vercel | Confirmar que el deploy incluye `afip-https-client.ts` con `https.request`, `ciphers: 'DEFAULT@SECLEVEL=1'` y `minDHSize: 512`. No usar solo `fetch` global hacia esos hosts. |
...
## QR de constatación AFIP (ticket térmico)

Además del CAE en PDF, Nexus genera la **URL del verificador** para el código QR que exige la constatación de comprobantes (misma especificación que AFIP / pyafipws `PyQR`).

| Elemento | Valor |
|---|---|
| URL base | `https://www.afip.gob.ar/fe/qr/?p=` |
| Parámetro `p` | Cadena **base64** (UTF-8) del JSON del comprobante |

El JSON incluye: `ver` (1), `fecha` (YYYY-MM-DD), `cuit` (numérico), `ptoVta`, `tipoCmp` (código WSFE), `nroCmp`, `importe`, `moneda` (`PES`), `ctz` (1), `tipoDocRec`, `nroDocRec`, `tipoCodAut` (`E` para CAE), `codAut` (CAE como número).

**Implementación:** función pura `buildQrArcaUrl` en `src/lib/facturacion/arca/qr.ts`, reutilizando `mapTipoComprobante` y `mapTipoDocReceptor` de `tipos.ts`. Tras obtener CAE en `emitirComprobante`, se calcula `qr_url` y se devuelve en la respuesta de `POST /api/facturacion/emitir` (no se guarda en base de datos).

**Impresión:** el POS (`ticket-termico.tsx`) codifica esa URL en un QR con **bwip-js** (SVG inline), tamaño fijo **25 mm** para papel 80/57 mm, y debajo imprime CAE y fecha de vencimiento del CAE.

**Tests:** `src/test/arca-qr.test.ts` valida el payload decodificado del `qr_url` para un caso de referencia.
...
| Archivo | Propósito |
|---|---|
| `src/lib/facturacion/arca/afip-https-client.ts` | POST HTTPS a WSAA/WSFE: `https.request` con `DEFAULT@SECLEVEL=1` y `minDHSize` (compatibilidad OpenSSL 3 + AFIP producción) |
| `src/lib/facturacion/arca/fetch-wsfe-retry.ts` | Reintentos HTTP hacia WSFE; delega en `afipHttpsPost` |
| `src/lib/facturacion/arca/wsaa.ts` | Autenticación WSAA: construir TRA, firmar CMS, obtener ticket, renovación automática; POST vía `afipHttpsPost` |
| `src/lib/facturacion/arca/wsfe.ts` | Operaciones WSFE: solicitar CAE, consultar último comprobante |
| `src/lib/facturacion/arca/qr.ts` | URL del verificador AFIP para QR de constatación (`buildQrArcaUrl`) |
