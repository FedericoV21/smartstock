...
**Importante:** el importador genérico de productos **no** aplica automáticamente el `proveedor.descuento_pct` del proveedor elegido: los costos de la planilla se graban tal cual. El descuento comercial del proveedor se usa solo en el módulo **analizador de rentabilidad** (listas de precios con preview y aplicación explícita).
...
flowchart TD
    ...
    subgraph Backend["Servidor"]
        ...
        U --> V[opcional: cuenta corriente proveedor si a crédito]
        V --> W[importacion_log por request]
        W --> X[Resumen final en cliente]
    end
```
...
| **Confirmación** | `src/lib/importar/client-import.ts`: `ejecutarImportacionPorLotes` parte el payload en chunks (tamaño por defecto **500** filas), llama varias veces a `POST /api/importar/ejecutar`, acumula contadores y **reindexa** `detalle_errores[].fila` respecto del total importado. |
| **Servidor por lote** | `src/lib/importar/ejecutar-importacion.ts`: al inicio de cada request se precargan productos existentes con **una** consulta `.in('codigo', …)` filtrada por `tenant_id`, `sucursal_id` y `proveedor_id` (o `proveedor_id` nulo), y se arma un `Map` en memoria. La clave de emparejamiento no es el código solo: es **código + nombre (descripción)** con el nombre normalizado (`trim` + minúsculas), de modo que dos productos con el mismo proveedor, mismo código y distinta descripción se crean o actualizan de forma **independiente**. Los errores guardan `valor_original` **truncado** para no inflar logs. Con proveedor, el preview puede enviar `cuenta_corriente_proveedor: 'credito'`: al final del lote se ajusta el saldo del proveedor en `cuenta_corriente` con la suma de (Δ stock × costo) por fila; `sin_impacto` (o sin proveedor) deja solo movimientos de stock. Re-ejecutar el mismo import puede volver a impactar deuda. |
...
### Productos pesables por nombre (opt-in) y columna código → PLU

En **`/importar/preview`** (`src/app/(dashboard)/importar/preview/page.tsx`) hay un checkbox **«Detectar productos pesables por nombre»**. **Por defecto está desmarcado:** la importación **no** infiere `unidad` `kg`/`gramo` ni `es_pesable` solo por el texto del nombre.

Si el usuario **lo marca** antes de confirmar:

1. Cada lote enviado a `POST /api/importar/ejecutar` incluye `aplicar_inferencia_pesable_por_nombre: true` (además de `filas`, `proveedor_id`, `origen`, etc.; ver `EjecutarImportacionClientPayload` en `src/lib/importar/client-import.ts`).
2. En el servidor, `ejecutarImportacionFilas` resuelve la unidad de stock con `resolverUnidadStockImportacion` (`src/lib/producto/inferir-unidad-stock-desde-texto.ts`), con este orden:
   - Valor de columna **Unidad** si coincide con un enum válido (`kg`, `gramo`, `unidad`, …).
   - Heurística sobre el **texto libre** de la columna Unidad (p. ej. «por kilo» en la celda).
   - **Solo si el flag es verdadero:** patrones conservadores en **nombre · texto de Unidad** (p. ej. `x kg`, `por kilo`, `× 2,5 kg`, `x 100 gr`). No se usa un simple «500 gr» en el nombre sin conector `x` / `por` / `×`.
3. Si la fila queda **pesable** (`unidad` `kg` o `gramo` y `es_pesable`), el valor de la columna mapeada como **`codigo_barras`** se guarda como **`plu`** (máximo **5** caracteres; si es más largo, la importación falla con mensaje explícito) y **`codigo_barras`** pasa a **`null`**.
4. En el **preview**, el encabezado **«Código barras / PLU»** y la etiqueta por fila **«PLU (pesable)»** solo se muestran cuando el checkbox está activo, para que la grilla coincida con lo que se aplicará al importar.

Sin el checkbox, el comportamiento de unidad/pesable vuelve a depender solo de la columna Unidad (enum o glosa), como antes de esta inferencia por nombre.

---

## Paso 6 — Ejecución del upsert en el servidor

La ruta real usa `getTenantSession`, `moduloGuard`, resuelve `sucursal_id` y delega en `ejecutarImportacionFilas` (`src/lib/importar/ejecutar-importacion.ts`). El cliente puede invocarla **varias veces** con subconjuntos de filas; ver `ejecutarImportacionPorLotes` en `src/lib/importar/client-import.ts`.

El cuerpo JSON puede incluir el campo opcional **`aplicar_inferencia_pesable_por_nombre`** (`boolean`, por defecto interpretado como `false` si no se envía). Cuando es `true`, aplica la lógica descrita en la sección anterior; si es `false` u omite, no se ejecuta la inferencia por nombre en `resolverUnidadStockImportacion`.

**Criterio “crear vs actualizar” en el servidor (fuente: `ejecutar-importacion.ts`):** para filas con código, se busca un producto existente en la misma **sucursal**, con el **proveedor** de la importación (o `proveedor_id` nulo, si la importación es sin proveedor) y con el **mismo código** y el **mismo nombre** (comparado en forma normalizada: recorte de espacios y comparación en minúsculas). Si coincide → se **actualiza** esa fila. Si no → se **crea** un producto nuevo. Así pueden coexistir, por ejemplo, dos filas con el mismo código y el mismo proveedor y descripciones distintas, como productos distintos.

**Reimportar listas de precios:** para que la segunda importación actualice al mismo registro, la descripción de la fila debe seguir alineada con el nombre almacenado (misma lógica de normalización). Si solo cambia el texto de la descripción, el sistema puede interpretar la fila como un artículo **nuevo**.
...
## Preferencia: unificar productos entre proveedores

Por defecto, el importador asume que un mismo `código` traído por proveedores distintos pertenece a productos distintos: cada proveedor mantiene su propio SKU. Para tenants con catálogos donde el mismo artículo se compra a varios proveedores, existe la preferencia de negocio `unificarProductosEntreProveedores` (configurable en **Configuración → Catálogo y proveedores**, con override por sucursal).

Cuando la preferencia está **activa** y la fila importada y el producto existente comparten **exactamente** `código` y `código de barras` (ambos no vacíos, comparados case-insensitive y con `trim`), el importador:

1. **No crea un duplicado**: actualiza la fila existente, aunque venga de otro `proveedor_id`.
2. **Conserva el `proveedor_id` original** del producto (queda como "proveedor principal"). El nuevo proveedor queda registrado en `movimiento.proveedor_id` y en `producto_lote_ingreso.proveedor_id`.
3. **El costo solo sube** (si la pref `precioCostoSoloSube` está activa, default `true`): si la fila trae un costo menor, se conserva el costo anterior y se recalcula `precio_venta` con la fórmula vigente.
4. **El stock se suma** como `entrada` (no como `ajuste`). La cantidad importada se agrega al stock previo y queda registrada con su proveedor.
5. **Cada ingreso genera un lote** (si `registrarLotesPorIngreso` está activa, default `true`) en `producto_lote_ingreso`, conservando cantidad, vencimiento, costo y proveedor del lote. La columna `producto.fecha_vencimiento` se sincroniza con el lote vivo más próximo a vencer mediante trigger (sólo al cambiar filas en `producto_lote_ingreso`; **no** al vender ni al pasar el calendario). Detalle para soporte en `docs/stock.md` («Alertas vs. varios lotes»).

Si la fila importada o el producto candidato **no tienen `código_barras`**, no se considera match cross-proveedor: se mantiene el comportamiento anterior (puede crearse un duplicado por proveedor distinto). Esto es intencional: barcode es la garantía de que se trata del mismo SKU físico.

La preferencia solo afecta **cargas futuras**: los productos duplicados que ya existen se mantienen como están; no se hace una "fusión retroactiva".

```mermaid
flowchart LR
  archivo[Archivo o factura proveedor B] --> upsert[Upsert importador]
  upsert --> pref{prefs.unificar_productos_entre_proveedores?}
  pref -->|"OFF (default)"| viejo[Match por proveedor_id<br/>= comportamiento actual]
  pref -->|ON| match[Match estricto<br/>código + código_barras]
  match --> existe{Match en otro proveedor?}
  existe -->|No| crear[Insert producto nuevo]
  existe -->|Sí| actualizar[Update producto existente<br/>preserva proveedor_id]
  actualizar --> costo[precio_costo solo sube]
  actualizar --> stock[registrar_movimiento entrada<br/>+ lote con vencimiento]
  crear --> lote_ini[Lote inicial si hay stock o vencimiento]
  stock --> mov[movimiento.proveedor_id = proveedor archivo]
```

El preflight (`POST /api/importar/preflight`) replica la misma lógica: cuando la pref está activa, no filtra por `proveedor_id` y resuelve match estricto código + barcode antes de pedir resolución de ambigüedad al usuario.

---

## Detección de duplicados

**En el preview (cliente):** si el archivo tiene el **mismo código** en dos o más filas válidas, antes de importar se abre un diálogo (`ImportarCodigoDuplicadoDialog`) para elegir:

- **Productos distintos**: se importa **una fila = un producto**, incluso si el nombre y el resto de los datos son idénticos.
- **Un solo producto**: se elige qué fila “gana” y el resto se omite en esta importación.

Ya no se hace un “deduplicado” automático por `código + nombre` en el cliente antes de ejecutar.

**En el servidor:** no hay unicidad estricta de `codigo` a nivel de tenant en la base (migración `054_producto_codigo_duplicado_permitido.sql`); el upsert de importación se resuelve por **sucursal + proveedor (o nulo) + código + nombre (normalizado)**, no solo por `(tenant, código)`.

### Actualización ambigua (2+ matches)

Si una fila coincide con **más de un** producto existente (mismo código y mismo nombre, dentro del contexto de sucursal + proveedor de la importación), el sistema pide resolución antes de ejecutar:

- **Actualizar**: el usuario elige qué `producto_id` actualizar.
- **Crear nuevo**: se inserta un producto nuevo aunque exista match.

El cliente hace un preflight (`POST /api/importar/preflight`) para detectar estas filas y abre un diálogo (`ImportarMatchAmbiguoDialog`) para resolverlas; luego envía `resolucion_match` al endpoint de ejecución.

```typescript
// La clave de match server-side usa claveProductoImport(codigo, nombre)
// (codigo trim, nombre trim + lower). Para filas ambiguas, se envía `resolucion_match`
// (clave: fila_original) con la decisión update(producto_id) o create.
```
