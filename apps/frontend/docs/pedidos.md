...
# Nexus — Módulo de Pedidos y Presupuestos
...
## Promociones (congeladas en el pedido)

Al **crear** un pedido (`POST /api/pedidos`) o **convertir un presupuesto en pedido**, se ejecuta el mismo motor de promos que en facturación, usando la **fecha del pedido** (`pedido.fecha`, normalmente hoy en AR). Cada `pedido_item` guarda precio efectivo, subtotal y las columnas snapshot de promo (`promocion_id`, `promocion_descripcion`, `precio_unitario_original`, `descuento_promo_monto`).

Al **facturar** (`POST /api/pedidos/[id]/facturar`), esos valores se copian al comprobante **sin recalcular** promos vigentes a la fecha de factura (`omitirAplicarPromociones` en `emitirComprobante`). Así el total facturado coincide con lo acordado en el pedido aunque el catálogo de promos cambie después.
