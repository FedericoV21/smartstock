...
**Reportes y métricas de producto:** ver [reportes.md](./reportes.md) (ventas por SKU vs tickets POS, vencimiento y última entrada de stock).
...
- **Foto opcional por producto** (`imagen_url`): subida desde la ficha del producto; se almacena una miniatura WebP en Supabase Storage y se muestra en la búsqueda y en el carrito del **POS** (`/facturacion/pos`). Detalle en [Foto del producto (POS)](#foto-del-producto-pos).
...
### Foto del producto (POS)

- **UI:** `/productos/[id]` — bloque «Foto para POS (opcional)»: selector de archivo, **Subir**, **Quitar** (requiere permiso de edición de stock; no visor).
- **API**
  - `POST /api/productos/:id/imagen` — `multipart/form-data` con campo `file`. Tipos: PNG, JPEG, WebP; máx. 2 MB. El servidor redimensiona con **sharp** (máx. 256 px en el lado largo, `fit: inside`), convierte a **WebP** (calidad ~80) y sube a Storage en `producto-imagenes` con path `{tenant_id}/{producto_id}/preview.webp` (`upsert`). Actualiza `producto.imagen_url` con la URL pública.
  - `DELETE /api/productos/:id/imagen` — elimina el objeto en Storage (si existe) y pone `imagen_url` en `null`.
- **PATCH /api/productos/:id** no incluye `imagen_url`: la URL solo se define por los endpoints anteriores (evita URLs arbitrarias).
- **Listado de productos** (`GET /api/productos`) y **búsqueda POS** (`GET /api/pos/buscar-productos`) devuelven `imagen_url` cuando existe. El escáner (`GET /api/productos/buscar-por-barcode`) ya expone el producto completo, incluida la imagen.

### Presentación de compra (caja, pack, peso por caja)

- El **stock** (`stock_actual`, movimientos, POS) sigue una sola unidad canónica: `producto.unidad`.
- Campos opcionales `unidad_compra` + `contenido_unidad_compra`: definen “1 caja = N unidades de stock” (o gramos si la base es `gramo`).
- **Entradas de stock:** en `POST /api/movimientos` se puede enviar `usar_unidad_compra: true` con **entrada** para que la cantidad se interprete en cajas/packs; el servidor multiplica por `contenido_unidad_compra` antes de `registrar_movimiento`. En la ficha de producto, el bloque “Movimiento rápido” ofrece casilla si hay presentación configurada.
- **Depósito del movimiento (multi-sucursal):** `POST /api/movimientos` acepta opcional `sucursal_id` (UUID). Debe ser una sucursal **operable** para el usuario; si no se envía, se usa `producto.sucursal_id` (hogar). La RPC `registrar_movimiento` (**106**) aplica el cambio en `stock_sucursal` de ese depósito. `GET /api/movimientos` lista por `movimiento.sucursal_id` (sucursal activa en sesión). En ficha, si hay varias filas en `stocks_sucursal`, “Movimiento rápido” deja elegir depósito y envía `sucursal_id`.
- **Importación** y **preview de listas de precios** pueden sugerir la presentación desde el nombre (heurísticas en `src/lib/producto/inferir-presentacion-desde-texto.ts`); las columnas explícitas del Excel ganan sobre el texto.

Referencia de bucket y migración: `docs/base-de-datos.md` (tabla `producto`, migración `062`).
...
## Lotes de ingreso (`producto_lote_ingreso`)

Cada vez que un producto recibe stock por importación, lectura de factura, alta manual o POS al vuelo, el sistema deja una fila en `producto_lote_ingreso` (migración `119_producto_lote_ingreso.sql`):

- `cantidad`, `fecha_vencimiento` y `precio_costo` del lote.
- `proveedor_id` (nullable) del proveedor que originó el ingreso.
- `origen` (`importacion` | `lector_facturas` | `manual` | `pos` | `comprobante_compra`).
- Enlaces a `importacion_log_id`, `lector_factura_log_id` y `movimiento_id` para trazabilidad.

Un trigger `trg_producto_lote_refresh_vencimiento` mantiene `producto.fecha_vencimiento` igual a la **fecha más próxima** entre los lotes vivos (cantidad > 0). Esto preserva el comportamiento de las alertas de vencimiento (`/api/alertas/vencimientos`) sin requerir migración de datos: los productos cargados antes de la pref siguen mostrando su `fecha_vencimiento` original; los que se cargan después con lotes muestran la del próximo lote a vencer.

La pestaña **Lotes / Vencimientos** en la ficha del producto (`/productos/[id]`) lista los lotes con proveedor, cantidad, costo, vencimiento y origen, permitiendo ver casos como "30 unidades del Proveedor A vencen el 12-2026 y 50 del Proveedor B vencen el 03-2027".

La generación de lotes se activa con la preferencia `registrarLotesPorIngreso` (default `true`) en `business_prefs`. Apagarla deja de crear lotes nuevos pero no toca los existentes.

### Alertas vs. varios lotes (comportamiento actual)

El dashboard (`VencimientosCard`) y `GET /api/alertas/vencimientos` **no leen la tabla `producto_lote_ingreso`**; sólo **`producto.fecha_vencimiento`**, en sucursales operables y con fecha dentro de los **30 días** próximos. Por producto aparece **un renglón**: la fecha utilizada para `días_restantes` y el estado (**vencido** / **crítico** ≤7 días / **próximo** entre 8 y 30 días) corresponden a ese único valor maestro.

Cuando hay lotes vivos (`cantidad` > 0 y `fecha_vencimiento` no nula), el trigger `trg_producto_lote_refresh_vencimiento` iguala **`producto.fecha_vencimiento`** al **mínimo** de las fechas de esos lotes. El detalle de cada partida sigue visible en la ficha (**Lotes / Vencimientos**); no hay una segunda alerta paralela por el siguiente vencimiento mientras el primero siga contando como mínimo.

**Cuándo se recalcula `producto.fecha_vencimiento`:** sólo después de **`INSERT`, `UPDATE` o `DELETE` en `producto_lote_ingreso`**. No hay tarea por calendario: **el sólo paso del día** después de una fecha ya vencida **no dispara** un salto automático “al próximo” vencimiento. Mientras siga habiendo un lote con esa fecha próxima y **`cantidad` > 0**, el **mínimo** sigue siendo esa fecha (incluso en el pasado si no se corrigen los datos), por lo que el producto puede seguir figurando como **vencido** hasta que algo cambie en los lotes o en **`producto.fecha_vencimiento`** a mano.

**Ventas y salidas de stock (`registrar_movimiento`):** actualizan `stock_actual` / `stock_sucursal` pero **no descuentan** la **`cantidad`** de **`producto_lote_ingreso`**. Ejemplo: 41 unidades en stock globales entre un lote de 1 que vence en una semana y otro de 40 que vence en un mes — si vendés **4 unidades**, **no** se asume FEFO ni se rebajan lotes; las filas de lote quedan como se cargaron y **`producto.fecha_vencimiento`** **no** pasa “automáticamente” al vencimiento del mes. Para que el mínimo refleje sólo stock con el segundo vencimiento hace falta **ajustar los lotes** (p. ej. llevar a 0 la `cantidad` del lote agotado, eliminar una fila, o proceso futuro de consumo por lote ligado al POS).

Si **no** hay lotes vivos con vencimiento, el trigger **no modifica** `producto.fecha_vencimiento` (siguen valiendo cargas sólo sobre el maestro — importación/manual sin filas recientes en lotes).

### `movimiento.proveedor_id`

`movimiento` ganó (migración `120_movimiento_proveedor_id.sql`) una columna opcional `proveedor_id` que `registrar_movimiento` rellena automáticamente cuando recibe `p_proveedor_id`. Eso permite filtrar entradas por proveedor sin pasar por `importacion_log` ni `comprobante` (útil para reportes de ingresos por proveedor incluso cuando se cargaron sin factura).
...
Única fecha por SKU en la lista: **`producto.fecha_vencimiento`**. Cuando coexisten varios **`producto_lote_ingreso`**, ese campo refleja el **próximo** vencimiento entre lotes **con cantidad > 0** sólo cuando el trigger se ejecuta tras un cambio en lotes (ver «Alertas vs. varios lotes» arriba). El listado filtrado `?vencidos=true` en productos usa el mismo criterio.
...
### Estado `activo` (proveedor inactivo)

- En base de datos, `proveedor.activo` (boolean, default `true`) desacopla al proveedor de los listados y selectores “habituales” sin borrarlo.
- **Pantalla `/proveedores`:** el filtro **Mostrar** es por defecto **Solo activos**; se puede ver **Solo inactivos** o **Todos**. El listado pide a la API el conjunto acorde (`estado=activos` | `inactivos` | `todos`).
- **Detalle `/proveedores/[id]`:** se puede marcar o desmarcar el checkbox de estado (o incluir **Proveedor activo** al guardar en modo edición). Un proveedor inactivo muestra la etiqueta **Inactivo** en el título.
- **POS** (`GET /api/pos/proveedores`): solo proveedores con `activo = true` (no usa el parámetro `estado` del listado general).
- Otras pantallas que necesitan el listado completo (p. ej. importar, reportes con filtro por proveedor, lector de facturas) consumen `GET /api/proveedores?estado=todos`.
- **Baja de productos al inhabilitar proveedor:** al pasar un proveedor a inactivo (`activo = false`), la base de datos (trigger `proveedor_inactivo_desactiva_productos`, migración `069`) marca **`producto.activo = false`** para todos los productos del mismo tenant cuyo **`producto.proveedor_id`** es ese proveedor y que seguían activos. No desactiva artículos que solo lo tengan como proveedor alternativo en **`producto_proveedor`**. **Reactivar el proveedor no reactiva** esos productos; hay que activarlos desde cada ficha o con otro proceso explícito.

### API Route

**`GET /api/proveedores`** (módulo `stock` vía `moduloGuard`, sesión con tenant)

- Query opcional: **`estado=activos`** (defecto si falta o el valor no es reconocido) | **`inactivos`** | **`todos`**. Filtra en servidor por `proveedor.activo`.
- Respuesta: `{ proveedores: Array<{ id, nombre, cuit, telefono, email, activo, … }> }` (incluye columnas y relación a cuenta corriente según `src/app/api/proveedores/route.ts`).

Implementación y permisos: `src/app/api/proveedores/route.ts` (`getTenantSession`, `moduloGuard('stock')`, visor sin escritura en POST).

Fragmento didáctico de **POST** (el código real además puede crear cuenta corriente y validar condiciones):
...
**`PATCH /api/proveedores/[id]`** acepta, entre otros campos, **`activo`** (boolean) para activar o desactivar al proveedor. Si se envía `activo: false`, el trigger en BD aplica la baja en cascada de productos descrita arriba (misma transacción que el `UPDATE` de `proveedor`).

### Detalle con perfil de mapeo Excel

```typescript
// src/app/api/proveedores/[id]/route.ts (esquema ilustrativo; el código usa getTenantSession y params async)
export async function GET(
  request: Request,
  { params }: { params: { id: string } }
) {
