...
**Integración ARCA (WSAA/WSFE) en Vercel:** el runtime usa **Node.js** con **OpenSSL 3**. Hacia los servidores de AFIP en producción no alcanza con `fetch` global: la app llama a AFIP con `https.request` y opciones TLS documentadas en [`arca.md`](./arca.md#cliente-http-y-tls-hacia-afip-nodejs-openssl-3-vercel) (`afip-https-client.ts`). Si tras un upgrade de Node en Vercel reaparecieran errores `dh key too small` / `EPROTO`, revisar esa sección antes de cambiar infraestructura.
...
| `GEMINI_API_KEY` | API key de Google Gemini (red final de extracción por visión si falla Open Router) | **No** | `AIza...` | v3.0 |
| `OPEN_ROUTER_API_KEY` | API key de [Open Router](https://openrouter.ai) para extracción por PDF/imagen (lector de facturas, listas IA) | **No** | `sk-or-...` | v3.0+ |
| `OPEN_ROUTER_MODELS` | Slugs de modelos separados por coma o espacio, en orden (fallback automático entre ellos) | **No** | `openai/gpt-4o,google/gemini-2.0-flash-001` | Con Open Router |
| `IA_VISION_PRIMARY` | `openrouter` (default si hay key + modelos), `gemini` (solo Gemini), `auto` | **No** | `openrouter` | Opcional |
| `OPEN_ROUTER_HTTP_REFERER` | URL de la app para atribución Open Router | **No** | `https://tu-dominio.com` | Opcional |
| `OPEN_ROUTER_APP_TITLE` | Nombre visible en rankings Open Router | **No** | `SmartStock` | Opcional |
| `OPEN_ROUTER_PDF_ENGINE` | Motor de parseo PDF en Open Router: `cloudflare-ai`, `mistral-ocr`, `native` | **No** | `cloudflare-ai` | Opcional |
| `OPENROUTER_API_KEY` | Alias opcional de `OPEN_ROUTER_API_KEY` (gana la variable con guiones bajos si ambas existen) | **No** | — | Opcional |
...
# Gemini (red final si Open Router agota modelos; v3.0+)
# GEMINI_API_KEY=AIza...

# Open Router — extracción por visión (facturas, listas PDF/imagen)
# OPEN_ROUTER_API_KEY=sk-or-v1-...
# OPEN_ROUTER_MODELS=openai/gpt-4o,anthropic/claude-3.5-sonnet,google/gemini-2.0-flash-001
# IA_VISION_PRIMARY=openrouter
# OPEN_ROUTER_PDF_ENGINE=cloudflare-ai
...
| `GEMINI_API_KEY` | `AIza...` | Production (recomendado como respaldo de visión) |
| `OPEN_ROUTER_API_KEY` | `sk-or-...` | Production (si se usa cadena de modelos OR) |
| `OPEN_ROUTER_MODELS` | lista ordenada | Production (junto con OR key) |
