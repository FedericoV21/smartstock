...
El módulo `ia_precios` (Plan Completo) permite extraer datos de productos desde listas de precios en formato PDF o imagen usando Gemini 1.5 Pro. Los datos extraídos entran al mismo pipeline de importación que el Excel (preview → validación → upsert), reutilizando toda la infraestructura existente. En el preview se puede elegir **proveedor** y, si aplica, **contado vs. a crédito** en cuenta corriente (`cuenta_corriente_proveedor`), con la misma lógica que el import Excel (ver `docs/importador.md`). Al confirmar, la importación se envía en **lotes** mediante `ejecutarImportacionPorLotes` (`src/lib/importar/client-import.ts`), igual que en el flujo Excel, para evitar requests demasiado grandes.

**Inferencia de pesables por nombre:** el flujo IA **no** muestra hoy el checkbox de `docs/importador.md` (productos pesables por patrones en el nombre y PLU desde columna código); el payload no envía `aplicar_inferencia_pesable_por_nombre`, por lo que en importaciones con `origen: ia_pdf` esa inferencia **no** se aplica salvo que se incorpore la misma opción en la UI de IA.
