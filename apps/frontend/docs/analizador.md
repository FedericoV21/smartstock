...
### Descuento por proveedor (analizador)

El proveedor puede tener `proveedor.descuento_pct` (acuerdo comercial). En el **preview** de la lista analizada, ese porcentaje se aplica por defecto sobre el **costo extraído** (`lista_precios_item.precio_lista`, valor del archivo) para calcular costos netos, variaciones, márgenes y precios sugeridos. El usuario puede **cambiar el % solo para esa lista** sin nueva ronda al servidor; al **aplicar**, el backend persiste el costo neto en `producto.precio_costo` y guarda un snapshot en `lista_precios.descuento_proveedor_pct_aplicado`.

Este descuento **no** aplica en el lector de facturas de compra, **no** aplica en el importador genérico de productos y **no** se aplica solo al elegir proveedor en el alta manual de productos (solo hay un hint opcional con botón).

Cuando una lista se aplica:

- Se actualiza `producto.precio_costo` con el **costo neto** (extraído × (1 − descuento%/100), redondeo a 2 decimales)
- Opcionalmente se actualiza `producto.precio_venta`
- Se registra un `precio_historial` con origen `lista_precios` (costo/venta anterior vs nuevo coherentes con el neto aplicado)
- Se hace upsert en `producto_proveedor` con el costo neto
- Se actualiza el estado de `lista_precios` y el snapshot `descuento_proveedor_pct_aplicado`
