...
| `lector_facturas` | Sí | Solo Plan Completo (en la app, el lector también es usable con `facturador_simple`; ver `docs/lector-facturas.md`) |
...
| `importador_excel` | `/importar`, `/importar/mapeo`, `/importar/preview` (checkbox opcional: pesables por nombre + PLU; ver `importador.md`) | `/api/importar/validar`, `/api/importar/ejecutar` |
| `facturador_simple` | `/facturacion`, `/facturacion/nueva`, `/facturacion/[id]`, `/facturacion/cierre-caja`, `/clientes`, `/clientes/[id]` | `/api/facturacion/emitir`, `/api/caja/*`, `/api/configuracion/cajas` |
...
| `lector_facturas` | `/lector-facturas`, `/lector-facturas/historial` | `/api/lector-facturas/extraer`, `confirmar`, `limite`, `logs` |
...
En `/configuracion/usuarios`, con plan **`base`** el tenant tiene un tope de **`USUARIOS_MAX_PLAN_BASE`** usuarios activos (hoy **5**); planes **intermedio** y **completo** no aplican ese tope en la app (detalle en `docs/autenticacion.md`).

**Lector de facturas:** el sidebar muestra el ítem si está activo **`lector_facturas`** o **`facturador_simple`** (`moduloAny` en `dashboard-chrome.tsx`). Las páginas y APIs usan `requireModuloAny` / `moduloGuardAny` con `MODULOS_ACCESO_LECTOR_FACTURAS` (`src/lib/modulos/modulo-key.ts`). Detalle en **`docs/lector-facturas.md`**.

**Código de barras y proveedor:** la unicidad de `codigo_barras` en productos activos depende de `proveedor_id` (ver `docs/base-de-datos.md`, migración `047`). El POS usa `GET /api/productos/buscar-por-barcode?codigo=…&proveedor_id=…`; sin proveedor, si hay varios productos con la misma barra la API responde **409** ambiguo. El alta por `POST /api/productos` exige proveedor cuando la barra ya existe en el tenant.
