...
Además hay **RBAC en base** (`permiso`, `rol_permiso`, `usuario_rol`, `usuario_permiso`) y la función `has_permiso`; cómo agregar claves nuevas y permisos extra por usuario está documentado en **[permisos-usuario-y-rbac.md](./permisos-usuario-y-rbac.md)**.
...
El tope depende del **`tenant.plan`** (`plan_tipo`: `base` | `intermedio` | `completo`):

| Plan | Usuarios activos (`usuario.activo = true`, `deleted_at` null) |
|------|------------------------------------------------------------------|
| **base** | Como máximo **`USUARIOS_MAX_PLAN_BASE`** (hoy **5**), definido en `src/lib/limits.ts` y aplicado vía **`getUsuariosMaxPorPlan`** |
| **intermedio** y **completo** | Sin tope en la app (`getUsuariosMaxPorPlan` devuelve `null`) |

La validación ocurre en **`POST /api/configuracion/usuarios`** (invitación por correo), **`POST /api/configuracion/usuarios/local`** (usuario local + PIN) y al **reactivar** un usuario en **`PATCH /api/configuracion/usuarios/[id]`** (si al pasar a `activo: true` se superaría el cupo del plan base). Respuesta **403** con mensaje explícito si ya no hay cupos. La pantalla muestra “X / N usuarios activos” cuando hay tope, o “X usuarios activos · sin límite” si no aplica. Desactivar un usuario (`activo = false`) libera cupo en plan base.

### Invitación de operadores y visores (al tenant existente)

El **POST** `src/app/api/configuracion/usuarios/route.ts` usa `auth.admin.inviteUserByEmail(email, { redirectTo })`. Supabase envía el correo de invitación; el `redirectTo` debe coincidir con la lista **Redirect URLs** del proyecto:

`{base}/api/auth/callback?next={encodeURIComponent('/invitacion/completar')}`

con `base = getPublicAppBaseUrl()` (`NEXT_PUBLIC_SITE_URL` / `NEXT_PUBLIC_APP_URL` / `VERCEL_URL`). Tras el intercambio de código en `/api/auth/callback`, el usuario define su contraseña en `/invitacion/completar`. Luego puede usar **Iniciar sesión** en `/login`. En la misma pantalla de login, el enlace “**Establecer contraseña**” apunta a `/recuperar-contrasena` (mismo flujo técnico que “olvidé mi contraseña”, distinto texto).

**Supabase (Auth → URL configuration):** Site URL alineada al dominio público; Redirect URLs que incluyan `/api/auth/callback` (p. ej. `https://<dominio>/api/auth/callback` o comodín `https://<dominio>/**`).
