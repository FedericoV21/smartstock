...
export async function POST(request: Request) {
  ...
  const { data: comp, error: compErr } = await session.supabase
    .from('comprobante')
    .select(
      'id, tenant_id, sucursal_id, estado, mp_point_intent_id, metodo_pago, total',
    )
    .eq('id', comprobanteId)
    .maybeSingle();
  ...
  if (!Number.isFinite(totalDb) || totalDb <= 0) {
    return NextResponse.json(
      { error: 'El borrador no tiene un total válido para cobrar en terminal' },
      { status: 400 },
    );
  }
  ...
  if (Math.abs(centsCliente - centsDb) > 2) {
    auditLogPosnet('iniciar_total_body_distinto_borrador', {
  ...
  auditLogPosnet('iniciar_solicitud', {
    comprobante_id: comprobanteId,
    tenant_id: session.tenantId,
    sucursal_id: comp.sucursal_id,
    total_body: total,
    total_comprobante_usado: totalDb,
    centavos_mp: centsDb,
  });
  ...
  if (cfg?.habilitado === false || !cfg?.device_id || !cfg?.access_token) {
    return NextResponse.json(
  ...
  try {
    ...
    auditLogPosnet('iniciar_intent_creado', {
      comprobante_id: comprobanteId,
      intent_id: intent.id,
      amount_centavos: cents,
      total_comprobante_pesos: totalDb,
    });
    ...
    return NextResponse.json({
      ...
      /** Total en pesos que se envió a MP (siempre el del borrador en BD). */
