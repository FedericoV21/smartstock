...
/** Días calendario desde hoyYmd hasta vencYmd (misma convención YYYY-MM-DD). Negativo = ya venció el día calendario. */
...
/** Facturas de venta que generan obligación de cobro en este módulo. */
export function esTipoFacturaVenta(tipo: string): boolean {
  return tipo === 'factura_a' || tipo === 'factura_b' || tipo === 'factura_c';
}

/**
 * Comprobantes de venta con cliente que registran deuda en `cobranza_factura`
 * (facturas fiscales y tickets POS).
 */
export function comprobanteGeneraRegistroCobranzaVenta(tipo: string): boolean {
  return esTipoFacturaVenta(tipo) || tipo === 'ticket';
}

export type CampanaCobranzaEstado = 'recordatorio_dia_5' | 'vencido' | 'saldo_cobranza_diaria';

/** Ventana de anticipación sugerida por condición de cobro. */
...
/**
 * Determina si una deuda debe mostrarse en la campana (recordatorio o vencida).
 * Reglas de negocio (días calendario en Argentina):
 * - Por defecto (resto de clientes): se muestra si faltan 2 días o menos hasta el vencimiento,
 *   o si ya venció el día calendario; sigue visible vencida hasta registrar pago.
 * - Si `clientePeriodicidadDiaria`: cuenta corriente en modalidad periódica con periodicidad diaria —
 *   se muestra **siempre** que haya saldo (no importa cuántos días falten para el próximo vencimiento
 *   de esa factura; respeta igual el snooze "recordar mañana").
 */
export function debeMostrarEnCampanaCobranza(params: {
  saldoPendiente: number;
  vencimientoAt: Date;
  recordatorioSnoozeUntil: Date | null;
  /** Modalidad periodico + periodicidad diaria en la cuenta corriente del cliente. */
  clientePeriodicidadDiaria?: boolean;
  /** Días de anticipación para el recordatorio previo al vencimiento. */
...
/** Dígitos con código país Argentina (54) para wa.me */
...
export function construirUrlWhatsAppCobranza(params: {
  ...
  const aviso =
    params.estado === 'vencido'
      ? 'Tenés un saldo vencido.'
      : params.estado === 'saldo_cobranza_diaria'
        ? 'Tenés cobranza diaria pactada en cuenta corriente; queda saldo pendiente en esta factura.'
