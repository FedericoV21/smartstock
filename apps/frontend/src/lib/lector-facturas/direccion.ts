...
function findClientePorCuitNorm(
  rows: { id: string; cuit_dni: string | null }[],
  cuitNorm: string | null,
): { id: string } | null {
  if (!cuitNorm) return null;
  for (const c of rows) {
    if (normCuit(c.cuit_dni) === cuitNorm) return c;
  }
  return null;
}
...
export async function detectarDireccion(
  ...
  const [{ data: proveedores }, { data: clientes }] = await Promise.all([
    supabase.from('proveedor').select('id, cuit, nombre').eq('tenant_id', tenantId),
    supabase.from('cliente').select('id, cuit_dni').eq('tenant_id', tenantId).eq('activo', true),
  ]);
