...
type CreateWorkerFn = (
  ...
  ) => Promise<{ data: { tsv?: string; text?: string; confidence?: number } }>;
...
async function cargarCreateWorker(): Promise<CreateWorkerFn> {
  // Evita que el bundler falle si la dependencia no está instalada en este entorno.
  ...
  try {
    ...
    if (typeof mod?.createWorker !== 'function') {
      throw new Error('Export createWorker no disponible');
    }
    return mod.createWorker;
  } catch {
    throw new Error(
      "No se pudo cargar OCR de tabla: falta instalar 'tesseract.js'. Ejecutá `npm install tesseract.js`.",
    );
  }
}
...
export async function extraerTablaFacturaConOcr(params: {
  ...
  const createWorker = await cargarCreateWorker();
  const langPath = join(
    process.cwd(),
    'node_modules',
    '@tesseract.js-data',
