...
} from '../../ai/utils/prompts';
import { esErrorExtraccionVisionIA, VisionIAError } from '../../ai/utils/vision-ia-error';
import {
  llamarVisionExtraccionJson,
  type VisionExtraccionMeta,
} from '../../ai/utils/vision-extraccion';
import {
  parsearJsonFacturaGemini,
  validarTotalesFactura,
  type FacturaGeminiPayload,
} from './factura-extraccion.util';
import {
...
} from './factura-multipagina.util';

export type { ArchivoFacturaEntrada, ArchivoFacturaResumen } from './factura-multipagina.util';
...
export function toCleanFacturaExtractorPayload(params: {
  archivoNombre: string;
  payload: FacturaGeminiPayload;
  validacion: ValidacionFactura;
  multipagina: Record<string, unknown>;
...
export async function extraerFacturaIaPura(params: {
  ...
  for (const hoja of hojas) {
    const r = await extraerHojaFactura(hoja, hojas.length, PROMPT_EXTRACCION_FACTURA);
    if (!r.ok) {
  ...
  return {
    ok: true,
    payload,
    clean: toCleanFacturaExtractorPayload({ archivoNombre, payload, validacion, multipagina }),
    validacion,
    archivoNombre,
    archivoMime,
    archivoTamano,
...
export function archivosDesdeExtractorJson(raw: unknown): ArchivoFacturaEntrada[] | null {
  ...
  for (let idx = 0; idx < rows.length; idx++) {
    ...
    const base64Raw =
      typeof r.base64 === 'string'
        ? r.base64
        : typeof r.data_base64 === 'string'
          ? r.data_base64
          : typeof r.contenido_base64 === 'string'
            ? r.contenido_base64
            : null;
    const mimeType =
      typeof r.mime_type === 'string'
        ...
        : typeof r.mimeType === 'string'
          ...
          : typeof r.type === 'string'
            ? r.type.trim()
            : null;
    if (!base64Raw || !mimeType) continue;
    const nombre =
      (typeof r.nombre === 'string' && r.nombre.trim()) ||
      (typeof r.name === 'string' && r.name.trim()) ||
      (typeof r.filename === 'string' && r.filename.trim()) ||
      `factura-${idx + 1}`;
    const clean = base64Raw.replace(/^data:[^;]+;base64,/i, '').replace(/\s+/g, '');
