...
import { hoyEnAR } from '../../cuenta-corriente/utils/periodo-reporte.util';
import type { AccessTokenPayload } from '../../auth/interfaces/access-token-payload.interface';
import { resolveAppRole } from '../../auth/utils/resolve-app-role';
import type { LectorFacturaLog } from '../entities/lector-factura-log.entity';
import type {
...
export function esAdminBorradoresLector(user: AccessTokenPayload): boolean {
  return (
    user.es_super_admin === true ||
    user.isSuperAdmin === true ||
    resolveAppRole(user) === 'admin'
  );
}
...
function parseValidacion(raw: unknown): LectorFacturaPreviewPayload['validacion'] {
  ...
  return {
    ...
    advertencias: Array.isArray(raw.advertencias) ? raw.advertencias.map(String) : [],
  };
}
...
export function validarPayloadBorradorLector(payload: unknown): LectorFacturaBorradorPayloadV1 {
  if (!isRecord(payload)) throw new BadRequestException('Payload de borrador invalido');
  if (payload.version !== 1) throw new BadRequestException('Version de borrador invalida');
  if (payload.paso !== 'preview') throw new BadRequestException('Paso de borrador invalido');
  if (typeof payload.archivoNombre !== 'string') {
    throw new BadRequestException('Archivo de borrador invalido');
  }
  if (payload.previewPaso !== 1 && payload.previewPaso !== 2) {
    throw new BadRequestException('Paso de preview invalido');
  }
  if (!isRecord(payload.extraccion)) throw new BadRequestException('Extraccion de borrador invalida');
  if (typeof payload.extraccion.log_id !== 'string') {
    throw new BadRequestException('Log de borrador invalido');
  }
  if (!isRecord(payload.draft)) throw new BadRequestException('Draft de borrador invalido');
  const draft = payload.draft as JsonRecord;
  if (!isRecord(draft.cabecera)) throw new BadRequestException('Cabecera de borrador invalida');
  if (!isRecord(draft.emisor)) throw new BadRequestException('Emisor de borrador invalido');
  if (!isRecord(draft.receptor)) throw new BadRequestException('Receptor de borrador invalido');
  if (!Array.isArray(draft.items)) throw new BadRequestException('Items de borrador invalidos');
  if (draft.tipoOperacion !== 'compra' && draft.tipoOperacion !== 'venta') {
    throw new BadRequestException('Tipo de operacion invalido');
  }
...
export function extraccionDesdeLog(row: JsonRecord, ivaDefault: number): LectorFacturaPreviewPayload {
...
export function payloadBorradorDesdeLog(
  ...
  const draft: LectorFacturaBorradorDraft = {
    ...
    impuestoInterno: numInput(
      extraccion.totales.impuesto_interno ?? extraccion.totales.otros_impuestos,
    ),
...
export function logEntityToJsonRecord(log: LectorFacturaLog, relations: LogRelations = {}): JsonRecord {
  return {
    id: log.id,
    tenant_id: log.tenantId,
    usuario_id: log.usuarioId,
    archivo_nombre: log.archivoNombre,
    archivo_mime: log.archivoMime,
    archivo_tamano: log.archivoTamano,
    datos_extraidos: log.datosExtraidos,
    direccion: log.direccion,
    proveedor_id: log.proveedorId,
    cliente_id: log.clienteId,
    created_at: log.createdAt.toISOString(),
    updated_at: log.updatedAt.toISOString(),
    proveedor: relations.proveedor ?? null,
    cliente: relations.cliente ?? null,
    usuario: relations.usuario ?? null,
  };
}
