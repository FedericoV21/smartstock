...
const UNIDADES_COMPRA: UnidadMedida[] = [
  UnidadMedida.caja,
  UnidadMedida.pack,
  UnidadMedida.unidad,
  UnidadMedida.kg,
...
export function inferirPresentacionDesdeTexto(
  ...
  const pesoOk = !unidadStock || unidadStock === UnidadMedida.gramo || unidadStock === UnidadMedida.kg;
  const cantOk =
    !unidadStock ||
    unidadStock === UnidadMedida.unidad ||
    unidadStock === UnidadMedida.caja ||
    unidadStock === UnidadMedida.pack ||
    unidadStock === UnidadMedida.litro ||
    unidadStock === UnidadMedida.ml ||
    unidadStock === UnidadMedida.metro;

  function contenidoDesdeKg(n: number): number {
    if (unidadStock === UnidadMedida.kg) return n;
    if (unidadStock === UnidadMedida.gramo || unidadStock == null) return n * 1000;
    return n * 1000;
  }

  function contenidoDesdeGramos(n: number): number {
    if (unidadStock === UnidadMedida.kg) return n / 1000;
    return n;
  }
  ...
  if (nxm) {
    ...
    if (n1 > 1 && n2 != null && n2 > 0) {
      ...
      if (esPeso && pesoOk) {
        ...
        return {
          unidad_compra: UnidadMedida.caja,
  ...
  if (packKgParen && pesoOk) {
    ...
    if (n != null && n > 0) {
      const u: UnidadMedida = lower.includes('pack') ? UnidadMedida.pack : UnidadMedida.caja;
      return {
        unidad_compra: u,
  ...
  if (gM && !lower.includes('kg') && pesoOk && unidadStock !== UnidadMedida.kg) {
...
export function inferirPresentacionPesoEnvaseDesdeTexto(
  texto: string,
  unidadStock: UnidadMedida.kg | UnidadMedida.gramo = UnidadMedida.kg,
): InferenciaPresentacion | null {
  const t = texto.replace(/\s+/g, ' ').trim();
  if (t.length < 2) return null;
  ...
  if (packKgParen) {
    const n = parseArgNumber(packKgParen[1]!);
    if (n != null && n > 0) {
      const u: UnidadMedida = lower.includes('pack')
        ? UnidadMedida.pack
