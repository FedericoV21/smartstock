...
} from './presentacion-compra.util';

const UNIDADES_LECTOR = new Set<string>(Object.values(UnidadMedida));
...
export function mapUnidadFacturaTexto(s: string | null | undefined): UnidadMedida {
  ...
  if (!raw) return UnidadMedida.unidad;
  ...
  const alias: Record<string, UnidadMedida> = {
    ...
    pack: UnidadMedida.pack,
    packs: UnidadMedida.pack,
    metro: UnidadMedida.metro,
    metros: UnidadMedida.metro,
    mt: UnidadMedida.metro,
    mts: UnidadMedida.metro,
  };
  const mapped = alias[v];
  if (mapped) return mapped;
  return UNIDADES_LECTOR.has(raw) ? (raw as UnidadMedida) : UnidadMedida.unidad;
}
