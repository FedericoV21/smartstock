import { GeminiError, llamarGemini } from './gemini';
import { llamarOpenRouterModel, parseOpenRouterModelsList, resolverOpenRouterApiKey } from './openrouter';
import { VisionIAError, type VisionIaAttempt } from './vision-ia-error';
...
function recoverableVisionIAError(e: VisionIAError): boolean {
  ...
  if (e.code === 'http') {
    ...
    if (status === 400) {
      return /model|overload|capacity|context|too long|unavailable|rate|busy|quota|token|length|invalid|provider returned error/i.test(
        e.message,
      );
    }
    return status == null;
...
export async function llamarVisionExtraccionJson(
  ...
  const primary = process.env.IA_VISION_PRIMARY?.trim().toLowerCase();
  const models = parseOpenRouterModelsList();
  const orKey = resolverOpenRouterApiKey();
  ...
  const runOpenRouterChain = async (): Promise<{ text: string; model: string } | null> => {
    ...
    for (const model of models) {
      try {
        const text = await llamarOpenRouterModel(prompt, archivo, model, { fileName: opts?.fileName });
        ...
        attempts.push({ provider: 'openrouter', model, ok: false, error: msg });
        if (e instanceof VisionIAError && recoverableVisionIAError(e)) continue;
  ...
  if (primary === 'openrouter') {
    if (!orKey) throw new VisionIAError('OPEN_ROUTER_API_KEY no configurada', 'config', { attempts });
    if (models.length === 0) throw new VisionIAError('OPEN_ROUTER_MODELS est├í vac├¡a', 'config', { attempts });
  }

  if (orFirst) {
    const orResult = await runOpenRouterChain();
    if (orResult) {
      return { text: orResult.text, meta: { provider: 'openrouter', model: orResult.model, attempts } };
    }
    if (geminiKey) {
      try {
        const r = await runGemini();
    ...
    throw new VisionIAError(
      'No hay proveedor de IA disponible: configur├í OPEN_ROUTER_MODELS y GEMINI_API_KEY, o al menos uno con modelos.',
      'config',
      { attempts },
    );
  }
