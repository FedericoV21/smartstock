function diasEnMesUtc(anio: number, mes: number): number {
  return new Date(Date.UTC(anio, mes + 1, 0)).getUTCDate();
}

function diaEfectivoEnMesUtc(anio: number, mes: number, diaDeseado: number): number {
  return Math.min(diaDeseado, diasEnMesUtc(anio, mes));
}

function toYmdUtc(y: number, m: number, d: number): string {
  const mm = String(m + 1).padStart(2, '0');
  const dd = String(d).padStart(2, '0');
  return `${y}-${mm}-${dd}`;
}

/** Próxima fecha de corte (UTC) >= hoy UTC, para día de corte 1-28. */
