...
export function inferMimeTypeForJob(params: {
...
export function isAudioDocumentType(documentType: string | null | undefined): boolean {
  const t = String(documentType ?? '').toLowerCase();
  return t === 'audio' || t === 'voice' || t === 'voice_note';
}

export function isMediaDocumentType(documentType: string | null | undefined): boolean {
  const t = String(documentType ?? '').toLowerCase();
