...
function parsearMontoInputUsuario(valor: string | null | undefined): number | null {
...
export function parseDisponibilidad(raw: unknown): Omit<DisponibilidadSemanal, 'id'>[] {
  if (!Array.isArray(raw)) {
    throw new BadRequestException('disponibilidad debe ser un array');
  }
  ...
  for (const item of raw) {
    ...
    if (!Number.isInteger(dia) || dia < 1 || dia > 7 || !ini || !fin) {
      throw new BadRequestException(
        'Cada disponibilidad requiere dia_semana 1-7, hora_inicio y hora_fin HH:mm',
      );
    }
    if (rangeDurationMinutes(ini, fin) == null) {
      throw new BadRequestException('hora_fin debe ser distinta a hora_inicio');
    }
  ...
  const rangos = rows.map((r) => {
    ...
    return {
      inicio,
      fin: finBase > inicio ? finBase : finBase + minutosSemana,
    };
  });
  ...
  for (let i = 0; i < rangos.length; i += 1) {
    for (let j = i + 1; j < rangos.length; j += 1) {
      ...
      if (
        ...
        throw new BadRequestException('Los rangos horarios de la agenda no pueden solaparse');
      }
    }
  }
...
export function parseExtrasHorarios(
  raw: unknown,
): (Omit<ExtraHorarioTurno, 'id'> & { descripcion: string | null })[] {
  if (!Array.isArray(raw)) {
    throw new BadRequestException('extras_horarios debe ser un array');
  }
  ...
  for (const item of raw) {
    ...
    if (!Number.isInteger(dia) || dia < 1 || dia > 7 || !ini || !fin) {
      throw new BadRequestException(
        'Cada extra requiere dia_semana 1-7, hora_inicio y hora_fin HH:mm',
      );
    }
    if (rangeDurationMinutes(ini, fin) == null) {
      throw new BadRequestException('La hora fin del extra debe ser distinta a la hora inicio');
    }
    if (extraMonto <= 0) {
      throw new BadRequestException('El monto extra debe ser mayor a 0');
    }
    rows.push({
      dia_semana: dia,
      hora_inicio: ini,
  ...
  for (let i = 0; i < rangos.length; i += 1) {
    for (let j = i + 1; j < rangos.length; j += 1) {
      ...
      if (
        ...
        throw new BadRequestException('Los extras horarios de la agenda no pueden solaparse');
      }
    }
  }
